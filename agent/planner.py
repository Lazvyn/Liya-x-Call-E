...
PLANNER_PROMPT = """You are the planning module of Liya, an autonomous AI agent.
...
- The user message will tell you the ACTUAL current date and time. ALWAYS use that
  exact value to compute any relative date/time ("tomorrow", "in 30 minutes", "next
  Monday", etc). NEVER guess, assume, or invent today's date from memory — the value
  given to you is authoritative and may differ from what you'd otherwise assume.
...
call_e
  task: string (required) — what the phone call should accomplish, in plain language
  phone: string (required) — recipient phone number, E.164 preferred (e.g. "+14155551234")
  region: string (optional, default "US") — recipient region code (US, SG, MY, IN, AE, AU, CA, GB, VN, DE, JP, FR, MX, BR, ID, PH, KE)
  locale: string (optional) — spoken language/locale, e.g. "en-US"
  wait: bool (optional, default true) — wait for the call to finish before returning
EXAMPLES:
...
Goal: "Call the dentist at +14155551234 and ask if they have any openings tomorrow afternoon"
Steps:

call_e | task: "Ask if the dentist's office has any openings tomorrow afternoon", phone: "+14155551234"
...
def _current_datetime_line() -> str:
    now = datetime.now()
    return f"Current date and time: {now.strftime('%A, %Y-%m-%d %H:%M')} (system clock)."
