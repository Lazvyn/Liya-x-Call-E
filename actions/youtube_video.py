...
def _handle_play(parameters: dict, player):
    ...
    if not query:
        return fail("Please tell me what you'd like to watch, sir.")
    ...
    # Partial outcome: a search page opened, but the specific video wasn't
    # confirmed playing — treat as success (real action was taken and the
    # user has something on screen), but keep the caveat in the message.
    return ok(f"Opened YouTube search for: {query} (manual selection required)")


def _handle_summarize(parameters: dict, player, speak):
    if not _TRANSCRIPT_OK:
        return fail("youtube-transcript-api is not installed. Run: pip install youtube-transcript-api")

    url = _ask_for_url("Please paste the YouTube video URL:")
    if not url:
        return fail("No URL provided, sir. Summary cancelled.")
    if not _is_valid_youtube_url(url):
        return fail("That doesn't appear to be a valid YouTube URL, sir.")

    video_id = _extract_video_id(url)
    if not video_id:
        return fail("Could not extract video ID from that URL, sir.")
    ...
    if not transcript:
        return fail("I couldn't retrieve a transcript for that video, sir.")
    ...
    try:
        ...
        return fail(f"Summary generation failed, sir: {e}")
    ...
    if parameters.get("save", False):
        ...
        return ok(f"Summary complete and saved to Desktop: {saved_path}")

    return ok(summary)


def _handle_get_info(parameters: dict, player, speak):
    url = parameters.get("url", "").strip()
    if not url:
        url = _ask_for_url("Please paste the YouTube video URL:")
    if not url or not _is_valid_youtube_url(url):
        return fail("Please provide a valid YouTube URL, sir.")

    video_id = _extract_video_id(url)
    if not video_id:
        return fail("Could not extract video ID, sir.")
    ...
    if not info:
        return fail("Could not retrieve video information, sir.")
...
def _handle_trending(parameters: dict, player, speak):
    ...
    if not trending:
        return fail(f"Could not fetch trending videos for region {region}, sir.")
