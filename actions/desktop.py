...
# Every negative-outcome prefix this module's internal functions can
...
def desktop_control(
    ...
    result = _desktop_control_impl(parameters, response, player, session_memory)
    return fail(result) if _is_error_result(result) else ok(result)


def _desktop_control_impl(
    parameters: dict = None,
    response=None,
    player=None,
    session_memory=None,
