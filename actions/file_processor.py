...
def _process_pdf(path: Path, action: str, params: dict, speak=None) -> str:
    ...
    if action in ("summarize", "extract_text", "translate_hint", "analyze", "reformat"):
        ...
        try:
            client   = _gemini_client()
            response = client.models.generate_content(
                model=MODEL, contents=prompt_map.get(action, f"Analyze:\n\n{text}")
            )
            result = response.text.strip()
...
def _process_text_doc(path: Path, file_type: str, action: str,
    ...
    try:
        client   = _gemini_client()
        response = client.models.generate_content(
            model=MODEL, contents=prompt_map[action]
        )
        result = response.text.strip()
        if len(result) > 600 and params.get("save", True):
            out = _output_path(path, action, ".txt")
...
# Every negative-outcome prefix this module's internal per-type handlers can
# return. Exact and authoritative for the static messages — not a guess —
# same pattern as file_controller.py.
#
# This file has ~20 different "<Verb> failed: {e}" messages across its nine
# type-specific handlers (resize/convert/compress/transcribe/trim/etc.), each
# with a different verb. Rather than enumerate every verb (and risk missing
# the next one added later), any string containing " failed:" is treated as
# a failure — a deliberate, documented exception to the prefix-only approach
# used elsewhere. This is safe here because success paths return either a
# fixed "<Thing> saved/converted/extracted..." message or free-form AI output
# that essentially never contains the literal substring " failed:".
_ERROR_PREFIXES = (
    ...
    "Please specify width, height, or scale.",
    "Could not extract text from PDF",
    "Could not extract text to convert.",
    "python-docx not installed.",
    "Unknown PDF action:",
    "File appears to be empty.",
    "pandas not installed.",
    "pydub not installed",
    "Unknown audio action:",
    "ffmpeg not found",
    "Execution timed out",
...
def _is_error_result(text: str) -> bool:
    return (
        any(text.startswith(p) for p in _ERROR_PREFIXES)
        or any(s in text for s in _ERROR_SUBSTRINGS)
    )


def file_processor(parameters: dict, player=None, speak=None):
    result = _file_processor_impl(parameters, player, speak)
    return fail(result) if _is_error_result(result) else ok(result)


def _file_processor_impl(parameters: dict, player=None, speak=None) -> str:
