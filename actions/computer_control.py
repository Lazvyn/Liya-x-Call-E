...
_ERROR_PREFIXES = (
    ...
    "focus_window (Windows) failed:",
    "focus_window (macOS) failed:",
    "focus_window (Linux) failed:",
    "focus_window (Linux) requires wmctrl or xdotool",
    "focus_window: unknown OS",
...
def computer_control(
    ...
    result = _computer_control_impl(parameters, response, player, session_memory)
    return fail(result) if _is_error_result(result) else ok(result)


def _computer_control_impl(
    parameters: dict,
    response=None,
    player=None,
    session_memory=None,
