...
def _search_flights_browser(
    ...
    def _text_of(res) -> str:
        # browser_control is itself migrated to ok()/fail(); unwrap its
        # message the same way the executor does for any other tool.
        if is_tool_result(res):
            return res["message"] if res["ok"] else ""
        return res or ""
    ...
    raw = _text_of(browser_control({"action": "get_text"}))
    return raw, url
...
def flight_finder(parameters: dict, player=None, speak=None):
    ...
    if not origin or not destination:
        return fail("Please provide both origin and destination, sir.")
    if not date_raw:
        return fail("Please provide a departure date, sir.")
    ...
    try:
        ...
        if not raw_text:
            return fail("Could not retrieve flight data, sir. The page may not have loaded.")
        ...
        # _format_spoken already returns a "couldn't find any flights"
        # message (not an exception) when the search legitimately came back
        # empty — that's a real failure outcome, not a successful step.
        if not flights:
            return fail(result)
        return ok(result)
