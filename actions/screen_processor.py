        ...
        while True:
            try:
                ...
                for exc in eg.exceptions:
                    print(f"[Vision] Session error: {exc}")
            finally:
                self._session = None
    ...
    async def _send_loop(self) -> None:
        while True:
            ...
            if not self._session:
                print("[Vision] No session — dropping image")
                continue
            try:
            ...
            async for response in self._session.receive():
                ...
                if sc.turn_complete:
                    if transcript and self._player:
                        ...
                        if full:
                            self._player.write_log(f"LIYA: {full}")
                            print(f"[Vision] {full}")
                    transcript = []
...
def screen_process(
    ...
    # Historically returned a bare bool that the executor's dispatcher
    # discarded outright (always logging a fixed success string regardless
    # of outcome — see agent/executor.py's "screen_process" branch). Now
    # returns a structured ToolResult so a real failure here is no longer
    ...
    if not user_text:
        ...
        return fail("No question provided for screen analysis.")
    ...
    try:
        ...
        return fail(f"Could not start vision session: {e}")
    ...
    try:
        ...
        return fail(f"Could not capture {angle}: {e}")

    _session.analyze(image_bytes, mime_type, user_text)
    return ok("Screen captured and analyzed.")
...
if __name__ == "__main__":
    ...
    time.sleep(10)
    print("Done." if result["ok"] else f"Failed: {result['message']}")