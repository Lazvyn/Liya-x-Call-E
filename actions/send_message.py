...
_SUCCESS_PREFIX = "Message sent to"
...
def send_message(
    ...
    if not receiver:
        return fail("Please specify a recipient.")
    if not message_text:
        return fail("Please specify the message content.")
    if not _PYAUTOGUI:
    ...
    return ok(result) if succeeded else fail(result)