...
def reminder(
    ...
    if not date_str or not time_str:
        raise ValueError("Reminder requires both a date and a time.")
    ...
    if target_dt <= datetime.now():
        # This is a logical failure (usually a bad date computed upstream by the
        # planner), not a successful outcome — it must raise so the executor's
        # failure/retry/replan path handles it instead of logging step.success.
        raise ValueError(
            f"Requested reminder time {target_dt.isoformat()} is in the past "
            f"(current time is {datetime.now().isoformat()}). "
            "The date/time passed to this tool must be in the future."
        )
    ...
    try:
        ...
        raise RuntimeError(f"Could not prepare the reminder script: {e}") from e
