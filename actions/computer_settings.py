...
def _detect_action(description: str) -> dict:
    ...
    from config.ai_client import generate, MODEL_FLASH_LITE
...
- Return ONLY the JSON, no explanation, no markdown."""
    ...
    try:
        resp = generate(MODEL_FLASH_LITE, prompt)
...
# Every negative-outcome prefix this module's dispatcher can return.
# Exact and authoritative — not a guess — same pattern as file_controller.py.
# Note: the confirm-before-dangerous-action prompt ("This will {action} the
# computer...") is deliberately treated as a failure too — the computer was
# NOT restarted/shut down, so logging it as step.success would be wrong.
_ERROR_PREFIXES = (
...
def computer_settings(
    ...
    result = _computer_settings_impl(parameters, response, player, session_memory)
    return fail(result) if _is_error_result(result) else ok(result)


def _computer_settings_impl(
    parameters: dict = None,
    response=None,
    player=None,
    session_memory=None,
