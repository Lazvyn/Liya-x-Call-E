...
# Every negative-outcome prefix this module's internal functions can return.
# Unlike agent/executor.py's global heuristic (which has to guess about
# arbitrary tools), this list is authoritative: these are ALL the failure
# strings this file produces, so matching against them is exact, not a guess.
_ERROR_PREFIXES = (
    "Access denied", "Path not found", "Not a directory",
    "Permission denied", "Error listing files", "Could not create file",
    "Could not create folder", "Not found:", "Protected directory",
    "Could not delete", "Source not found", "No destination specified",
    "Could not move", "Could not copy", "Could not rename",
    "Could not write file", "Search error", "Could not get disk usage",
    "Could not organize desktop", "Could not get file info",
    "Unknown action", "File controller error", "Search path not found",
)
...
def file_controller(
    ...
    try:
        ...
        result = f"File controller error ({action}): {e}"
