...
> repo itself was trimmed down to calling-only scope — everything
> except `call_e`/`call_status`/`save_memory` was removed from
> `actions/`, `main.py`'s tool surface, and `agent/adk_tools.py` — so
...
> longer exist) — see its note for the current equivalent.
...
2. A Google ADK agent (`agent/adk_agent.py`) wrapping the *same* action
   ...
   **Scope of the ADK path at the time this was written:** 10 of the
   ...
   `agent/adk_tools.py` — `web_search`, `file_controller`, `open_app`,
   ...
   from this repo entirely — this paragraph is left as written for
...
calling-only cut described at the top of this file — down to just the
...
`Dockerfile` installs — no PyQt6/pyautogui/Playwright) is unaffected,
...
earlier as submission cleanup — none were part of the skill's
...
> at the top of this file — `web_search`, `file_controller`, and
...
### Running it (historical — see note above)
