...
# ---------------------------------------------------------------------------
# Colour palette — classic hacker/terminal red theme
# ---------------------------------------------------------------------------
class C:
    BG        = "#050202"
    PANEL     = "#170606"
...
# ---------------------------------------------------------------------------
# HUD canvas — animated orb / face  (red terminal palette)
# ---------------------------------------------------------------------------
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        # subtle red grid dots
        p.setPen(QPen(qcol("#200909"), 1))
        ...
        if self._face_px:
            ...
            oc    = (120, 10, 10) if self.muted else (255, 40, 40)
...
class _DropCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        bg_col = qcol(
            "#3d0a0a" if z._drag_over else ("#200909" if z._hovering else C.PANEL),
            190,
        )
...
class SetupOverlay(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._key_input.setStyleSheet(f"""
            QLineEdit {{
                background: #170606; color: {C.TEXT};
    ...
    def _sel(self, key: str):
        self._sel_os = key
        pal = {
            "windows": (C.PRI,   "#2a0808"),
            "mac":     (C.ACC2,  "#220a0a"),
            "linux":   (C.GREEN, "#0a1a0f"),
        }
        for k, btn in self._os_btns.items():
            if k == key:
                ...
                btn.setStyleSheet(f"""
                    QPushButton {{
                        background: #170606; color: {C.TEXT_DIM};
...
class MainWindow(QMainWindow):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._spdm_mem = Speedometer("MEM",  "%",    "#ff5a5a")
        self._spdm_net = Speedometer("NET",  "MB/s", "#ff3131")
        self._spdm_gpu = Speedometer("GPU",  "%",    "#ff8a8a")
        self._spdm_tmp = Speedometer("TEMP", "°C",   "#e63946")
    ...
    def _build_center_bottom(self) -> QWidget:
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                background: {rgba("#170606", 210)}; color: {C.WHITE};
    ...
    def _style_mute_btn(self):
        if self._muted:
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {rgba("#2a0d0d", 220)}; color: {C.MUTED_C};
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {rgba("#220a0a", 220)}; color: {C.GREEN};
                    border: 1px solid {C.GREEN}; border-radius: 16px;
                }}
                QPushButton:hover {{ background: {rgba("#3d0a0a", 230)}; }}
            """)
