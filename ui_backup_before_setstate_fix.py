...
# ---------------------------------------------------------------------------
# Colour palette — classic hacker/terminal dark-green theme
# ---------------------------------------------------------------------------
class C:
    BG        = "#020a02"
    PANEL     = "#061906"
...
class Speedometer(QWidget):
    ...
    def paintEvent(self, _):
        ...
        if fill_span != 0:
            ...
            # glow effect — crisper layering: tight bright core, wide soft bloom
            for gw, ga in [(16, 22), (11, 45), (7, 90), (4, 160), (2, 235)]:
...
# ---------------------------------------------------------------------------
# HUD canvas — animated orb / face  (dark-green terminal palette)
# ---------------------------------------------------------------------------
class HudCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        # subtle green grid dots
        p.setPen(QPen(qcol("#0a2410"), 1))
        ...
        if self._face_px:
            ...
            oc    = (10, 60, 20) if self.muted else (40, 255, 80)
...
class _DropCanvas(QWidget):
    ...
    def paintEvent(self, _):
        ...
        bg_col = qcol(
            "#0f3d18" if z._drag_over else ("#0a2410" if z._hovering else C.PANEL),
            190,
        )
...
class SetupOverlay(QWidget):
    ...
    def _sel(self, key: str):
        self._sel_os = key
        pal = {
            "windows": (C.PRI,   "#082008"),
            "mac":     (C.ACC2,  "#082008"),
            "linux":   (C.GREEN, "#04140f"),
        }
        for k, btn in self._os_btns.items():
            if k == key:
                ...
                btn.setStyleSheet(f"""
                    QPushButton {{
                        background: #040f04; color: {C.TEXT_DIM};
...
class MainWindow(QMainWindow):
    ...
    def _build_left_panel(self) -> QWidget:
        ...
        self._spdm_mem = Speedometer("MEM",  "%",    "#5aff8a")
        self._spdm_net = Speedometer("NET",  "MB/s", "#31ff6a")
        self._spdm_gpu = Speedometer("GPU",  "%",    "#8affc2")
        self._spdm_tmp = Speedometer("TEMP", "°C",   "#46e67a")
    ...
    def _build_center_bottom(self) -> QWidget:
        ...
        self._input.setStyleSheet(f"""
            QLineEdit {{
                background: {rgba("#040f04", 210)}; color: {C.WHITE};
    ...
    def _style_mute_btn(self):
        if self._muted:
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {rgba("#132213", 220)}; color: {C.MUTED_C};
            ...
            self._mute_btn.setStyleSheet(f"""
                QPushButton {{
                    background: {rgba("#0a2410", 220)}; color: {C.GREEN};
                    border: 1px solid {C.GREEN}; border-radius: 16px;
                }}
                QPushButton:hover {{ background: {rgba("#0f3d18", 230)}; }}
            """)
